"""Readiness checks for v9 multi-source signal integration."""
...
def main() -> int:
  ...
  if integrated["raw_count"] != 7:
    errors.append("統合前の raw count を正しく集計できていません")
  if integrated["deduped_count"] >= integrated["raw_count"]:
    errors.append("重複除去が機能していません")
  if integrated["ranked_count"] > 100:
    errors.append("Top100 ranking 上限を超えています")

  sample = integrated["signals"][0]
  for field in (
    "signal_id",
    "run_id",
    "source_type",
    ...
    if field not in sample:
      errors.append(f"統合Signalに必須拡張フィールド {field} がありません")

  previous = []
  for row in integrated["signals"]:
    copied = dict(row)
    copied["current_rank"] = int(row["current_rank"]) + 1
    copied["summary"] = str(row.get("summary", "")) + " old"
    previous.append(copied)
  updated = apply_signal_change_tracking(integrated["signals"], previous)
  if not all(item.get("previous_rank") is not None for item in updated):
    errors.append("previous_rank を付与できていません")
  if not any(item.get("change_status") in {"Rising", "Updated", "Dropped", "Stable"} for item in updated):
    errors.append("change_status を計算できていません")

  top_by_source = build_source_top_signals(integrated["signals"], top_n=5)
  if len(top_by_source["patent"]) > 5 or len(top_by_source["paper"]) > 5:
    errors.append("情報源別Top5を制御できていません")
  ...
  if "integrate_multi_source_signals(" not in app_source or "apply_signal_change_tracking(" not in app_source:
    errors.append("signal_watch_app に統合 service が接続されていません")
  if "情報源別Top5" not in tabs_source:
    errors.append("情報源別Top5 UI が不足しています")

  if errors:
    for message in errors:
      print(f"[v9 signal integration readiness] ERROR: {message}")
    return 1
  print("[v9 signal integration readiness] OK: multi-source signal integration is ready.")
  return 0
