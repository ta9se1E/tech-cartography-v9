...
**Core Validation / 別テーマ検証（Phase 24.4 / 24.4A）**:

- 「炭素繊維以外のテーマでも流れが再現できるか」を **レポート** タブの **別テーマ検証** で確認
- dry-run → 既存 outputs 検証 → Manual Claims テンプレート → レポート保存の順で説明
- 「外部検索は同意チェック必須。UI からメール送信・scheduler 登録はしません」
...
- 「Web Signal Link は、NEDO/JST/METI などの高品質ソースと、特許・論文の技術テーマの重なりを見るものです。ただし、これは最終結論ではなく Link Candidate です」
- 「Phase 23.4.1 では、広いキーワード（PAN 等）だけで過剰に高スコアにならないように調整しています」
...
### 日本語Digestでの説明台本（Phase 24.1.1）

関係者への説明例:

> これは Tech Cartography が毎週お届けする**週次インテリジェンスDigest**の下書きです。
> 掲載内容はすべて**確認候補**であり、最終結論ではありません。
> 今週は「次に原典を確認すべき重点監視候補」と「新しく増えたシグナル」の差分を中心にまとめています。
> FTO・侵害・有効性の判断には使用せず、技術者・専門家が次に何を確認すべきかのたたき台としてご覧ください。
