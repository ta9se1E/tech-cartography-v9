"""End-to-end multi-source signal pipeline tests for v9."""
...
def test_end_to_end_multi_source_signal_pipeline() -> None:
  profile = _watch_profile()
  plan = build_unified_search_plan(profile)
  assert "plans" in plan and "global_web_plan" in plan
  ...
  assert integrated["raw_count"] == 9
  assert integrated["deduped_count"] < integrated["raw_count"]

  signals = integrated["signals"]
  assert {signal["type"] for signal in signals} >= {"patent", "paper", "web", "company"}
  assert all(signal["source_url"] for signal in signals)
  assert all(signal["data_origin"] for signal in signals)
  assert all(signal["record_stage"] == "staged" or signal["data_origin"] == "base_signal" for signal in signals)
  assert all(signal["retrieval_mode"] == "real" or signal["data_origin"] == "base_signal" for signal in signals)
  ...
  assert patent_external_ids.count("US2024000001A1") == 1

  paper_external_ids = [signal["external_id"] for signal in signals if signal["type"] == "paper"]
  assert paper_external_ids.count("10.1000/test1") == 1

  web_duplicate_groups = [signal["duplicate_group"] for signal in signals if signal["type"] == "web"]
  assert len(web_duplicate_groups) == len(set(web_duplicate_groups))

  organizations_top10 = [signal["organization"] for signal in signals[:10] if signal.get("organization")]
  assert organizations_top10.count("Toyota") < len(organizations_top10)

  top10 = select_diverse_top_signals([Signal.from_dict(signal) for signal in signals], top_n=10)
  assert len(top10) <= 10

  by_source = build_source_top_signals(signals, top_n=5)
  assert len(by_source["patent"]) <= 5
  assert len(by_source["paper"]) <= 5
  assert len(by_source["web"]) <= 5
  assert len(by_source["company"]) <= 5
  ...
  assert len(digest_top3) <= 3
  assert "Tech Cartography v9 週次ダイジェスト" in digest
  assert "人間レビュー状況" in digest

  assert patent_rows == original_patent
  assert paper_rows == original_paper
  assert web_company_rows == original_web_company
