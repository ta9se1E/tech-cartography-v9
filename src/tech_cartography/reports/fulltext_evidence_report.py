...
def build_fulltext_evidence_summary(result: dict[str, Any]) -> dict[str, Any]:
  ...
  return {
    ...
    "fulltext_scope": result.get("fulltext_scope", "claims_only"),
    "maximum_fulltext_usd": result.get("maximum_fulltext_usd", 10.0),
    "allow_expensive_fulltext": result.get("allow_expensive_fulltext", False),
    "cost_guard_requires_expensive_count": result.get("cost_guard_requires_expensive_count", 0),
    "blocked_by_usd_count": result.get("blocked_by_usd_count", 0),
...
def render_fulltext_evidence_markdown(summary: dict[str, Any]) -> str:
  lines = [
    ...
    f"- fulltext scope: {summary.get('fulltext_scope', 'claims_only')}",
    "",
  ...
  for record in summary.get("retrieved", []):
    ...
    lines.extend(
      [
        ...
        f"- fulltext_scope: {record.get('fulltext_scope', summary.get('fulltext_scope', 'claims_only'))}",
        f"- evidence_level: {record.get('evidence_level')}",
        f"- cost_guard_status: {record.get('cost_guard_status', 'n/a')}",
        f"- estimated_gb: {record.get('estimated_gb', 'n/a')}",
        f"- estimated_usd: {record.get('estimated_usd', 'n/a')}",
  ...
  lines.extend(
    [
      ...
      f"- maximum USD: {summary.get('maximum_fulltext_usd', 10.0)}",
      f"- allow expensive fulltext: {summary.get('allow_expensive_fulltext', False)}",
      f"- cost guard requires expensive confirmation: {summary.get('cost_guard_requires_expensive_count', 0)}",
      f"- blocked by USD guard: {summary.get('blocked_by_usd_count', 0)}",
      "",
      "claims_only では請求項だけを先に取得します。明細書は後で必要に応じて取得できます。",
      "GB上限を超えてもUSD見積もりが許容範囲なら、--allow-expensive-fulltext で明示承認後に実行できます。",
      "",
      "## Next Phase",
      "",
    ],
