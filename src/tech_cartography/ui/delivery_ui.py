...
def load_delivery_artifacts(
  ...
  pub = publication_number or DEFAULT_PUB
  ddir = _delivery_dir_for_pub(root, pub)

  paths = {
    "overview_page_md": ddir / "overview_page.md",
    "intelligence_report_md": ddir / f"intelligence_report_{pub}.md",
    "weekly_digest_md": ddir / f"weekly_digest_preview_{pub}.md",
    "weekly_digest_html": ddir / f"weekly_digest_preview_{pub}.html",
    "digest_diff_md": ddir / f"digest_diff_{pub}.md",
    "report_bundle_zip": ddir / f"tech_cartography_report_bundle_{pub}.zip",
  }
...
def render_intelligence_report_section(artifacts: DeliveryUIArtifacts) -> None:
  ...
  if artifacts.intelligence_report_md:
    ...
    st.download_button(
      label="Download intelligence_report.md",
      data=artifacts.intelligence_report_md,
      file_name=f"intelligence_report_{artifacts.publication_number}.md",
      mime="text/markdown",
      key="dl_intelligence_report",
    )
  else:
    st.info(
...
def render_weekly_digest_preview_section(artifacts: DeliveryUIArtifacts) -> None:
  ...
  if artifacts.weekly_digest_md:
    ...
    st.download_button(
      ...
      file_name=f"weekly_digest_preview_{artifacts.publication_number}.md",
      mime="text/markdown",
      key="dl_weekly_digest_md",
    )
  if artifacts.weekly_digest_html:
    st.download_button(
      label="Download weekly_digest_preview.html",
      data=artifacts.weekly_digest_html,
      file_name=f"weekly_digest_preview_{artifacts.publication_number}.html",
      mime="text/html",
      key="dl_weekly_digest_html",
    )
  elif not artifacts.weekly_digest_md:
    st.info("Weekly Digest Preview がまだありません。初回は initial snapshot として生成されます。")


def render_report_bundle_zip_section(artifacts: DeliveryUIArtifacts) -> None:
...
def render_delivery_section(
  ...
  *,
  ...
  st.divider()
  render_intelligence_report_section(artifacts)
  st.divider()
  render_weekly_digest_preview_section(artifacts)
  st.divider()
  render_report_bundle_zip_section(artifacts)
