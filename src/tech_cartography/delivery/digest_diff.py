...
def render_digest_diff_markdown(
  ...
  *,
  ...
  if diff.unchanged_summary:
    lines.extend(["## No Major Changes", "", "Summary hash unchanged since previous snapshot.", ""])
  ...
  _section("New Strategic Watch Items", diff.added_watch_items)
  if diff.removed_watch_items:
    lines.extend(["## Removed Strategic Watch Items", ""])
    for item in diff.removed_watch_items[:10]:
      lines.append(f"- {item}")
    lines.append("")

  _section("New Web Signals", diff.added_web_signals)
  _section("New Patent × Paper × Web Links", diff.added_links)
  _section("New Paper Evidence Candidates", diff.added_papers)
